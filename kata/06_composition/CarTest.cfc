component extends="testbox.system.BaseSpec" {
	...
	function run( testResults, testBox ){
		...
		describe("Car Spec", function() {
			...
			it("Car should stop by decelerating and depressing the brakes", function() {
				var engine = getMockBox().prepareMock(new Engine());
				engine.$("decelerate");
				var brakes = getMockBox().prepareMock(new Brakes());
				brakes.$("depress");
				var car = new Car(
					engine = engine,
					brakes = brakes
				...
				expect(engine.$once("decelerate")).toBe(true);
				expect(brakes.$once("depress")).toBe(true);
			});
